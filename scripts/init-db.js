...
stopsA.forEach(s => stopInsert.run(r1.lastInsertRowid, s.name, s.lat, s.lng, s.order));
stopsB.forEach(s => stopInsert.run(r2.lastInsertRowid, s.name, s.lat, s.lng, s.order));
stopsC.forEach(s => stopInsert.run(r3.lastInsertRowid, s.name, s.lat, s.lng, s.order));

// Users
const userInsert = db.prepare('INSERT INTO users (name, email, password_hash, role, route_id, phone) VALUES (?,?,?,?,?,?)');
userInsert.run('Admin Kumar',        'admin@college.edu',      hash('admin123'),     'admin',          null,                null,               '+91-9000000001');
userInsert.run('Transport Head Rao', 'transport@college.edu',  hash('transport123'), 'transport_head', null,               '+91-9000000002');
...
userInsert.run('Sneha Patel',    'student3@college.edu', hash('student123'), 'student', r2.lastInsertRowid, '+91-9000000008');
